...
FP=0;
recall=0;
[num,txt,raw1]=xlsread('train2.xls');%读取训练数据
...
% train_data(:,32:end)=[];%第一个问题删除后半部分数据，不带用药
% train_data(:,end-10:end-4)=[];
% train_data(:,29:35)=[];%删除前半部分数据

clear raw1
[num,txt,raw9]=xlsread('test2.xls');%读取测试数据
raw13=raw9;
test_data=raw13;
test_label=raw13(:,end);
test_data(:,end)=[];
% test_data(:,32:end)=[];%第一个问题删除后半部分数据,不带用药
...
for index1=1:size(train_data,1)
    for index2=1:size(train_data,2)
        if ismember(index2,[2,13,14,32:45])~=1
%           if ismember(index2,[2,13,14])~=1%此行针对原始问题和欠采样
...
for index1=1:size(test_data,1)
    for index2=1:size(test_data,2)
        if ismember(index2,[2,13,14,32:45])~=1%此行针对原始问题和欠采样
%            if ismember(index2,[2,13,14])~=1%此行针对原始问题和欠采样
