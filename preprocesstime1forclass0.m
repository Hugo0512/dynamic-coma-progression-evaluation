...
[num,txt,raw1]=xlsread('externalmissforest0.xls');%读取训练数据,这里只用清醒患者的数据，1类是清醒
[num,txt,raw2]=xlsread('external0.xlsx');%读取训练数据中的结局时间，即第几天清醒
...
%     end
    ...
    for index1=1:targetvalue
        ...
        if targetvalue<=daythreshold+1
             extendedvalue{count,39}=0;%1是清醒，0是不清醒
