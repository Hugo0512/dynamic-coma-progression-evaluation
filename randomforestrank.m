...
FP=0;
recall=0;
[num,txt,raw1]=xlsread('downsampletrain3.xls');%读取训练数据
...
train_data(:,40:end)=[];%删除后半部分数据，不带用药
% train_data(:,1:29)=[];%删除前半部分数据

clear raw1
[num,txt,raw9]=xlsread('test3.xls');%读取测试数据
raw13=raw9;
test_data=raw13;
test_label=raw13(:,end);
test_data(:,end)=[];
test_data(:,40:end)=[];%删除后半部分数据,不带用药
...
for index1=1:size(train_data,1)
    for index2=1:size(train_data,2)
        if ismember(index2,[2,16,17,40:46])~=1
...
colnames(40:end)=[];%删除后半部分属性名，不带用药
...
%  end
 ...
 for index=1:size(originaltestdata,2)
     ...
     if isempty(find(digitalindex,index))==1
%      if index~=7 && index~=8
